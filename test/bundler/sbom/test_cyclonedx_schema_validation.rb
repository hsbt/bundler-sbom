...
class CycloneDXSchemaValidationTest < Minitest::Test
  include TestHelper
  ...
  def test_valid_cyclonedx_document_with_single_gem
  ...
  def test_valid_cyclonedx_document_with_multiple_gems
    gem_data = [
  ...
  def test_valid_cyclonedx_document_with_no_license
    sbom = generate_sbom([{name: "unknown-gem", version: "1.0.0", licenses: []}])
    errors = schema.validate(sbom.to_hash).to_a
    assert_empty errors, errors.map { |e| "#{e["data_pointer"]}: #{e["type"]} - #{e["details"]}" }.join("\n")
  end

  def test_valid_cyclonedx_document_with_non_spdx_license_ids
